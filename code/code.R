...
datedata = longdata %>%
	mutate(category = str_replace_all(longdata$category, "_", " "),
		     date = strptime(as.integer(date), format = "%Y%m%d"))
datedata = filter(datedata, date > '1975-12-31')
...
ggplot(datedata, aes(x = as.Date(date), y = value+.1)) + 
	...
	xlab("Date") + 
	ylab("Count") + 
	ggtitle("All Cancer-Related Patents (1976-2016)") + 
	theme_dark()
 	ggsave("../figures/patent_timeseries_log2.png",width = 7, height = 5)
